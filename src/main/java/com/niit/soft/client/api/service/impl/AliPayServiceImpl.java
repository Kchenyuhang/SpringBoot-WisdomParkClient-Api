...
public class AliPayServiceImpl implements AliPayService {
    ...
    @Override
    public String AliPay(Double amount,  String jobNumber , String rechargeType, Integer regexId) throws AlipayApiException {
        ...
        if (regexId == 1) {
            ...
            request.setBizContent(JSON.toJSONString(data));
            SysOrder sysOrder =
                    SysOrder.builder().description(rechargeType).jobNumber(jobNumber).orderMoney(amount).orderNumber("10" + RandomUtil.randomInt(80000, 1000000)).isDeleted(false).gmtCreate(Timestamp.valueOf(LocalDateTime.now())).orderType("支出").payMethod("支付宝扫码").status(true).build();
            orderRepository.save(sysOrder);
            String response = alipayClient.pageExecute(request).getBody();
            return response;
        }
